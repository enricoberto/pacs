...
public :
  ...
  matrid (std::vector<double> &vec1, 
		  ...
		  std::vector<double> &vec3):  N(vec2.size()), a(vec1), b(vec2), c(vec3), gamma_set(false){
		gamma.resize(vec2.size(),0.0);
  }
...
*/
  ...
  double  *   mod_gamma () { return &(gamma[0]); };
